#!/usr/bin/env bash
# V5: dump-cache fixes + 1 prop/step + no screenshots + agent pre-clean
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"; cd "$ROOT"
SERIAL="${SERIAL:-5SM0125606000291}"
MINS="${MINS:-5}"; MAXSTEP="${MAXSTEP:-60}"; THROTTLE="${THROTTLE:-200}"
STAMP_RUN=$(date +%Y%m%d_%H%M%S)
LOG="$ROOT/modeA_runs/multi_decomp_v5_${STAMP_RUN}.log"
SUMMARY="$ROOT/modeA_runs/props_out/MULTI_DECOMP_V5_${STAMP_RUN}.tsv"
echo -e "pkg\tout\tsteps\tprops\tfired\tfire_pct\texecs\tdecomp_execs\tfails\terrors\tcrash\twidgets" > "$SUMMARY"
echo "RUN_V5 $STAMP_RUN mins=$MINS" | tee "$LOG"
packs_for() {
  case "$1" in
...
run_one() {
  ...
  out="$ROOT/modeA_runs/props_out/${pkg}_multiv5_${stamp}"
  mkdir -p "$out"
  packs=$(packs_for "$pkg")
...
for pkg in "${QUEUE[@]}"; do
  if ! hdc list targets 2>/dev/null | rg -q .; then
    echo "NO_DEVICE"; sleep 10
  fi

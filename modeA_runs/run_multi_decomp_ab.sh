...
LOG="$ROOT/modeA_runs/multi_decomp_${STAMP_RUN}.log"
SUMMARY="$ROOT/modeA_runs/props_out/MULTI_DECOMP_${STAMP_RUN}.tsv"
...
echo "RUN $STAMP_RUN mins=$MINS maxstep=$MAXSTEP" | tee "$LOG"
...
packs_for() {
  case "$1" in
    ...
    com.ss.hm.ugc.aweme|com.kuaishou.hmapp|com.youku.next|com.phoenix.read.next)
      echo "properties.modeA_props.decompiled properties.modeA_props.bug_find properties.modeA_props.flow properties.modeA_props.video_feed";;
    *)
      echo "properties.modeA_props.decompiled properties.modeA_props.bug_find properties.modeA_props.flow";;
  esac
}

# queue: skip calculator last? do diverse order — skip already-heavy ctrip/idlefish mid
QUEUE=(
  com.amap.hmapp
...
for pkg in "${QUEUE[@]}"; do
  ...
  out="$ROOT/modeA_runs/props_out/${pkg}_multidecomp_${stamp}"
  mkdir -p "$out"
  packs=$(packs_for "$pkg")

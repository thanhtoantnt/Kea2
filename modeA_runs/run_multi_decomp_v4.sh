#!/usr/bin/env bash
# V4: V2 lean setup + careful dismiss + agent pre-clean each app + retry on HdcError
set -u
ROOT="$(cd "$(dirname "$0")/.." && pwd)"; cd "$ROOT"
SERIAL="${SERIAL:-5SM0125606000291}"
MINS="${MINS:-4}"; MAXSTEP="${MAXSTEP:-50}"; THROTTLE="${THROTTLE:-350}"
STAMP_RUN=$(date +%Y%m%d_%H%M%S)
LOG="$ROOT/modeA_runs/multi_decomp_v4_${STAMP_RUN}.log"
SUMMARY="$ROOT/modeA_runs/props_out/MULTI_DECOMP_V4_${STAMP_RUN}.tsv"
echo -e "pkg\tout\tsteps\tprops\tfired\tfire_pct\texecs\tdecomp_execs\tfails\terrors\tcrash\twidgets" > "$SUMMARY"
echo "RUN_V4 $STAMP_RUN" | tee "$LOG"
packs_for() {
  case "$1" in
...
QUEUE=(
  ...
  com.sankuai.hmeituan com.phoenix.read.next com.youku.next com.xunmeng.pinduoduo.hos
  com.kuaishou.hmapp com.ss.hm.ugc.aweme com.huawei.hmos.calculator
  com.ctrip.harmonynext com.taobao.idlefish4ohos
)
run_one() {
  local pkg="$1" stamp out packs ec
  stamp=$(date +%Y%m%d_%H%M%S)
  out="$ROOT/modeA_runs/props_out/${pkg}_multiv4_${stamp}"
  mkdir -p "$out"
  packs=$(packs_for "$pkg")
  ...
  ec=$?
  if [[ $ec -ne 0 ]] && rg -q "HdcError|operation not permitted|Unexpected Error" "$out/console.log"; then
    echo "RETRY $pkg hdc" | tee -a "$LOG"
    hdc -t "$SERIAL" shell "rm -f /data/local/tmp/agent.so" >/dev/null 2>&1 || true
    sleep 4
    stamp=$(date +%Y%m%d_%H%M%S)
    out="$ROOT/modeA_runs/props_out/${pkg}_multiv4_${stamp}"

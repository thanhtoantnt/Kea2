#!/usr/bin/env bash
# Mode A v3: lean decomp packs + multi-pass dismiss + safer clickable + guest overlay
set -u
...
LOG="$ROOT/modeA_runs/multi_decomp_v3_${STAMP_RUN}.log"
SUMMARY="$ROOT/modeA_runs/props_out/MULTI_DECOMP_V3_${STAMP_RUN}.tsv"
...
echo "RUN_V3 $STAMP_RUN mins=$MINS maxstep=$MAXSTEP" | tee "$LOG"
...
for pkg in "${QUEUE[@]}"; do
  ...
  out="$ROOT/modeA_runs/props_out/${pkg}_multiv3_${stamp}"
  mkdir -p "$out"
  packs=$(packs_for "$pkg")
  echo "[$(date -Iseconds)] START $pkg out=$out" | tee -a "$LOG"
...
Path(summary).open("a").write(
    f"{pkg}\t{out}\t{st}\t{n}\t{fired}\t{fire_pct}\t{te}\t{de}\t{tf}\t{terr}\t{int(crash)}\t{wg}\n"
)
print(f"{pkg} steps={st} fire={fired}/{n}={fire_pct}% execs={te} decomp={de} fails={tf} crash={int(crash)}")
PY
